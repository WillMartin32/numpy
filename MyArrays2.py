import numpy as np

#row = student (student0, 1, 2, 3)
#col = exam (test0, 1, 2)
grades = np.array([[87, 96, 70], [100, 87, 90], [94, 77, 90], [100, 81, 82]])


student1 = grades[1]

student0_test1 = grades[0,1]

students0_1 = grades[0:2] #upper bound not included (only 0 and 1)

students1and3 = grades[[1,3]]

# to select students 1 and 3 but only test 2
students1and3_test2 = grades[[1,3],2]

all_students_test0 = grades[:,0]

all_students_test1_2 = grades[:,1:3]

all_students_test0and2 = grades[:,[0,2]]

'''
Use NumPy random-number generation to create an array of twelve
random grades in the range 60 through 100, then reshape the result
into a 3-by-4 array. Calculate the average of all the grades,
the averages of the grades for each test and the averages of the
grades for each student.
'''
grades = np.random.randint(60,101,12).reshape(3,4)

grades.mean()

#avg by col
grades.mean(axis=0)

#avg by row
grades.mean(axis=1)

numbers = np.arange(1,6)

#SHALLOW COPY
numbers_view = numbers.view()

numbers[1] *= 10 #Chaging the original changes the copy and visversa

numbers_view[1] /= 10

number_slice = numbers[0:3]

numbers[1] *= 20

#DEEP COPIES
numbers_copy = np.numbers.copy

numbers[1] *= 10 #Now changes don't affect the original copy because it's a deep copy

grades = np.array([[87, 96, 70], [100, 87, 90]])

#RESHAPE method produces a shallow copy
grades_reshaped = grades.reshape(1,6)

#RESIZE method changes the array
grades.resize(1,6)

#FLATTEN creates a deep copy
flattended = grades.flatten()

#RAVEL creates a shallow copy
raveled = grades.ravel()

raveled[0] = 100

flattended[1] = 100

grades = np.array([[87, 96, 70], [100, 87, 90]])

grades.T #Flips the array

grades2 = np.array([[94,77,90],[100,81,82]])


#Adds more columns
h_grades = np.hstack((grades, grades2))

#Adds more rows
v_grades = np.vstack((grades, grades2))


print()